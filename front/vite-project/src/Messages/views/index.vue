...
<template>
    <section class="bg-[#FFFFF] border border-gray-200 p-4 shadow-sm rounded-2xl h-full flex flex-col gap-3 hable__messages">
        ...
        <div class="w-full self-end flex gap-3 items-center">
            <VInput type="text" @on-enter-event="clearInput" :on-enter-args="[selectedChannel, text]" :on-enter="sendMessage" placeholder="Digite a mensagem" v-model="text" />
            <VButton background-color="#5D5FEF" icon="fa-greater-than" @click="sendMessage(selectedChannel, text); text =''"/>
        </div>
